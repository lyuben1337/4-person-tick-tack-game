...
UiModal.Header = function UiModalHeader({ children, className }) {
  return (
    <div className={clsx(className, "-px-6 pl-6 pt-6 pb-4 text-2xl")}>
      {children}
    </div>
  );
};
...
UiModal.Body = function UiModalBody({ children, className }) {
  return (
    <div className={clsx(className, "-px-6 pl-6 pb-4 text-2xl")}>
      {children}
    </div>
  );
};
...
export function UiModal({
  ...
  const modal = (
    <div
      onClick={handleClick}
      className={clsx(
        "overflow-y-auto fixed inset-0 bg-slate-900/60 backdrop-blur-sm pt-10 pb-10",
        className,
      )}
    >
      <div
        data-id="modal"
        className={clsx(
          "flex flex-col bg-white rounded-lg min-h-[320px] mx-auto relative",
          {
      ...
      >
        <button
          onClick={onClose}
          className="hover:bg-white/40 transition-colors w-8 h-8 flex rounded items-center justify-center bg-white/10 absolute top-0 left-[calc(100%+12px)]"
        >
